class Populator {
	...
	populatePrizes(prizes) {
		...
		prizes.forEach((prize) => {
			...
			heading.innerText = prize.name;
			heading.style.color = "white";
			const subheading = document.createElement("h3");
			subheading.style.color = "white";
			subheading.innerText = "Sponsored by " + prize.sponsor;
	...
	populateSchedule(schedule) {
		...
		schedule.forEach((event) => {
			const entry = event.time + " | " + event.description;
	...
	populateStands(projectList) {
		const table = document.getElementById("teams-table");

		this.sortProjects(projectList);

		// console.log(projectList);

		var inner = "<th>TEAM NAME</th>" +
		"<th>PROJECT TITLE</th>" +
		"<th>STAND NUMBER</th>";
		projectList.forEach(project => {
			const row = this.generateRow(project);
			console.log(row);
			inner += row;
		});
	...
	generateRow(project) {
		const html = '<tr style="border-top: 1px solid whitesmoke">' + 
		"<td>" + project.team + "</td>" +
		"<td>" + project.name + "</td>" +
		"<td>" + project.standNo.toString() + "</td>" +
		"</tr>"
		return html;
	}
	...
	sortProjects(projectList) {
		projectList.sort((a, b) => {
			const numA = a.standNo;
			const numB = b.standNo;
			if (numA < numB) {
				return -1;
			}
			if (numA > numB) {
...
class COSEvent {
	constructor(time = "time", description = "event") {
		this.time = time;
		this.description = description;
	}
}
...
function init() {
	console.log("Performing initialisation");
...
function populateScheduleHelper() {
	const testProjectList = new ProjectList();
	const testPopulator = new Populator(testProjectList);
	const schedule = testProjectList.getSchedule();
	testPopulator.populateSchedule(schedule);
	testPopulator.populateStands(testProjectList.generateList());
}
...
class ProjectList {
	...
	getPrizes() {
		...
		const agile = new Prize(
			"Agile Process Adoption",
			"Agile Bridge",
			"The Agile process adoption should stand out for the following attributes:\n- Usage of personas\n- How their solution provides value\n- Agile project management tool usage\n- Good breakdown of epics and stories\n- Continuous updates to documentation"
		);
		prizes.push(agile);
		const algorithmic = new Prize(
			"Algorithmic Innovation",
			"Retro Rabbit",
			"Algorithmics forms the underpinnings of computer science. It is about designing correct, efficient and implementable algorithms for real-world problems. This prize will be awarded to the team who applied the most brilliant algorithmic ideas to address scalability and efficiency issues"
		);
		prizes.push(algorithmic);
		const architecture = new Prize(
			"Architectural Awareness",
			"Entelect",
			"Software architecture refers to the high-level structures of a software system. Candidates for this prize should convince the judges that their system design is based on appropriate software architecture and design patterns to ensure compliance with the quality requirements of their system."
		);
		prizes.push(architecture);
		const data = new Prize(
			"Best Use of Data",
		...
		prizes.push(data);
		const userexperience = new Prize(
	...
	getSchedule() {
		...
		const setup = new COSEvent("7:30 - 8:30", "Setup (teams and sponsors)");
		const opening = new COSEvent("9:00 - 9:05", "Event Opening (Prof Hein Venter)");
		const showcase = new COSEvent("9:05 - 12:00", "Project showcase and grading");
		const announcement = new COSEvent(
			"12:00 - 12:15",
			"Announcement of prize winners"
		);
		const thanks = new COSEvent("12:15 - 12:30", "Thank you to the sponsors");
		const closing = new COSEvent("12:30 - 13:00", "Event closing (Prof Hein Venter)");
		const photo = new COSEvent("13:00 - 13:30", "Class photo");

		schedule.push(setup, opening, showcase, announcement, thanks, closing, photo);
